...
class CubaLIFNode(BaseNode):
    ...
    @staticmethod
    @torch.jit.script
    def jit_single_step_forward_soft_reset(x: torch.Tensor, i: torch.Tensor, v: torch.Tensor, v_threshold: float):
    ...
    @staticmethod
    @torch.jit.script
    def jit_single_step_forward_hard_reset(x: torch.Tensor, i: torch.Tensor, v: torch.Tensor, v_threshold: float, v_reset: float):
    ...
    def single_step_forward(self, x: torch.Tensor):
        ...
        if self.v_reset is None:
            ...
            spike, self.i, self.v = self.jit_single_step_forward_hard_reset(x, self.i, self.v, self.v_threshold, self.v_reset)
    ...
    @staticmethod
    @torch.jit.script
    def jit_multi_step_forward_soft_reset_with_i_v_seq(x_seq: torch.Tensor, i: torch.Tensor, v: torch.Tensor, current_decay: torch.Tensor, voltage_decay: torch.Tensor, v_threshold: float):
        spike_seq = torch.zeros_like(x_seq)
    ...
    @staticmethod
    @torch.jit.script
    def jit_multi_step_forward_hard_reset_with_i_v_seq(x_seq: torch.Tensor, i: torch.Tensor, v: torch.Tensor, current_decay: torch.Tensor, voltage_decay: torch.Tensor, v_threshold: float, v_reset: float):
        spike_seq = torch.zeros_like(x_seq)
        i_seq = torch.zeros_like(x_seq)
        v_seq = torch.zeros_like(x_seq)
        for t in range(x_seq.shape[0]):
    ...
    if self.training:
        ...
        if self.v_reset is None:
            if self.store_i_v_seq:
                spike_seq, self.i, self.v, self.i_seq, self.v_seq = self.jit_multi_step_forward_soft_reset_with_i_v_seq(x_seq, self.i, self.v, self.current_decay, self.voltage_decay, self.v_threshold)
            else:
            ...
            if self.store_i_v_seq:
                spike_seq, self.i, self.v, self.i_seq, self.v_seq = self.jit_multi_step_forward_hard_reset_with_i_v_seq(x_seq, self.i, self.v, self.current_decay, self.voltage_decay, self.v_threshold)
            else:
                spike_seq, self.i, self.v = self.jit_multi_step_forward_hard_reset(x_seq, self.i, self.v, self.current_decay, self.voltage_decay, self.v_threshold)
